...
forceAtomToString (GrzAtomInt a) = T.pack $ show a
forceAtomToString (GrzAtomBool True) = "True"
forceAtomToString (GrzAtomBool False) = "False"
forceAtomToString (GrzAtomString a) = a
forceAtomToString (GrzAtomFile a) = "File " `T.append` (T.pack $ show a)

-- | An atom prett printer.
ppAtom :: GrzAtom -> GrzString  
ppAtom (GrzAtomInt a) = T.pack $ show a
ppAtom (GrzAtomBool True) = "True"
ppAtom (GrzAtomBool False) = "False"
ppAtom (GrzAtomString a) = "\"" `T.append` (T.pack $ show a) `T.append` "\""
ppAtom (GrzAtomFile a) = "File " `T.append` (T.pack $ show a)
...
objWrapperToString :: (Typeable o, GrzObjClass o) => o -> GrzString
objWrapperToString obj = T.pack $ last $ splitOn "." (show $ typeOf obj)
...
ppAtomBox :: GrzAtomBox -> GrzString
ppAtomBox box =
    T.intercalate "\n\n" $ map ppAtomPair (Map.toList box)
    
ppAtomPair :: (GrzKey,[GrzAtom]) -> GrzString
ppAtomPair (k,v) = "        " `T.append` k `T.append` ": \n            " `T.append` (ppAtomList v) 

ppAtomList v = T.intercalate ", " (map ppAtom v)

-- | Pretty prints an object in a short form.
ppObj :: GrzObjClass o => o -> GrzString
ppObj = ppObj' . unwrapObj

ppObj' :: GrzObj -> GrzString
ppObj' (GrzObjID i) = "object " `T.append` (T.pack $ show i)
ppObj' obj = (getType obj)
                `T.append` " "
                `T.append` (T.pack $ show $ getID obj) 

-- | Pretty prints an object in a detailed form.
ppObjFull :: GrzObjClass o => o -> GrzString
ppObjFull = ppObjFull' . unwrapObj

ppObjFull' :: GrzObj -> GrzString
ppObjFull' (GrzObjID i) = "object " `T.append` (T.pack $ show i)
ppObjFull' obj = (getType obj)
                `T.append` " "
                `T.append` (T.pack $ show $ getID obj) 
                `T.append` " {\n\n"
                `T.append` "    timeCreated: \t"
                `T.append` (T.pack $ convertTime $ getTimeCreated obj)
                `T.append` "\n    timeUpdated: \t"
                `T.append` (T.pack $ convertTime $ getTimeUpdated obj)
                `T.append` "\n    owner: \t\t"
                `T.append` (T.pack $ show $ getOwner obj)
                `T.append` "\n    container: \t\t"
                `T.append` (T.pack $ show $ getContainer obj)
                `T.append` "\n    site: \t\t"
                `T.append` (T.pack $ show $ getSite obj)
                `T.append` "\n    enabled: \t\t"
                `T.append` (if isEnabled obj then "True" else "False")
                `T.append` "\n\n    metadata:\n\n"
                `T.append` (ppAtomBox $ getMetadata obj)
                `T.append` "\n}\n"           
     where
...
class GrzFieldsClass r where 
    fields' :: GrzAtomKeyClass a => GrzAtomKey a -> [GrzString] -> r

instance GrzFieldsClass [GrzString] where
...
noMetadata :: [GrzAtomKey GrzString]        
noMetadata = []

-- | can be used in place of fields when you want all metadata returned from the database
allMetadata :: [GrzAtomKey GrzString]
allMetadata = [GrzAtomKey { atomKey = "*" }]
